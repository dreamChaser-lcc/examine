...
const CustomTable: FC<ITableProps> = (props) => {
  ...
  const option = [
    ...
    { values: 'user2', label: 'user2' },
    { values: 'uesr3', label: 'user3' },
  ];
  const handleClick = () => {
    console.log();
    const change: object[] = [
  ...
  const leftOperation: ReactNode[] = [
    <Radio.Button key="btu" value="large">
      标记已读
    </Radio.Button>,
    <Checkbox key="check" onChange={checkBoxChange}>
      只看未读
    </Checkbox>,
  ];
  //右边操作栏
  const rightOperation: ReactNode[] = [
    <Form form={formRef} layout="inline" key="select">
      <Form.Item name="select">
        ...
        >
          {option.map((item) => {
            return (
              <Select.Option value={item.values} key={item.values}>
                {item.label}
              </Select.Option>
            );
          })}
    ...
    <Radio.Button key="reset" onClick={reset} value="large">
      重置
    </Radio.Button>,
  ];
  //点击行获取详情
    ...
    <div className="customTb-container">
      <Row className="tableHead" justify={'space-between'} align={'middle'}>
        ...
        <Col id="right" span={12}>
          <div className="right">
            <Space size="middle">{rightOperation.map((item) => item)}</Space>
          </div>
        </Col>
      </Row>
      <Table
